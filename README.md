...
## Si GitHub Actions échoue sur "Setup Node"

Erreur vue: `No existing directory found containing cache-dependency-path="frontend/yarn.lock"`.

C'est corrigé dans le workflow. Il faut juste:
1. mettre à jour la branche,
2. relancer **Actions > Build Android APK > Run workflow**.


## IMPORTANT - utilise ce workflow précis

Dans GitHub Actions, choisis **Build Android APK (MANUEL SIMPLE V2 - NPM ONLY)**.

Si tu lances un ancien workflow, tu retomberas sur l'erreur `frontend/yarn.lock`.

Nouveau correctif: le workflow manuel n'utilise plus Yarn, il utilise `npm ci` (NPM ONLY).


## Si erreur "Invalid package.json in package.json"

Le workflow manuel a été corrigé: il passe par **npm** au lieu de Yarn.
Relance **Build Android APK (MANUEL SIMPLE V2 - NPM ONLY)** après mise à jour de la branche.


## Vérification anti-erreur de branche

Dans les logs du workflow, tu dois voir: `WORKFLOW_V2_NPM_ONLY`.
Si ce texte n'apparaît pas, tu n'exécutes pas le bon workflow/branche.
